...
### 0.1 微信开发者工具全流程返工记录（2026-06-02）

- 本地小程序联调用微信开发者工具打开 `uniapp/dist/dev/mp-weixin`；`uniapp/dist/build/mp-weixin` 使用生产接口，只用于发布包验收。
- 订单创建后先进入待确认状态，需后台或服务人员确认档期后，订单详情页才会出现“支付定金”入口。
- 余额支付回归必须至少连续覆盖两笔支付，防止固定 `transaction_id=BALANCE` 被历史支付记录占用后阻断后续订单。
- 小程序联调时同步核对数据库：订单支付状态、`la_payment`、`la_financial_flow` 和用户余额必须一致。
- 若控制台出现 HTTP 图片/字体资源告警，应优先改为 HTTPS 或可配置域名；暂不能统一替换时按资源治理缺陷登记。
- 企业微信通知出现 `SSL certificate problem: unable to get local issuer certificate` 时，按运维配置问题处理，优先配置 CA 证书路径，不把关闭 SSL 校验作为默认修复。
...
专项口径：详见 `docs/prd-questionnaire-full-link-acceptance-20260523.md` 第 7 节验收清单。

- 支付完成/订单完成后待填问卷入口出现。
- 问卷列表、详情、提交失败均有错误提示/重试或返回路径。
- 后台问卷分页、筛选、详情不崩溃；特别关注分页 `v-model` 运行时错误。
- 同一订单只生成一条新人问卷任务；补发只更新发送次数，不新增任务。
- 已填写/已取消任务不可再次提交或补发，用户端展示明确状态。
- 必填题后端必须拦截，前端应提示具体未填题目。
- 已提交答案必须保存题目快照；模板升级、题库删除不影响历史答案展示。
- 发送失败/未订阅时，按最终产品规则验证用户仍可从订单详情或问卷列表进入，或明确待推送不可见。
