...
const drawTextBlock = ({
    ...
    const svg = safeLines
        .map(
            (line, index) =>
                `<text x="${x}" y="${y + index * lineHeight}" text-anchor="${textAnchor}" font-size="${fontSize}" font-weight="${fontWeight}" fill="${fill}"${getTextFontAttr(fontFamily)}${letterSpacingAttr}>${escapeXml(line)}</text>`
        )
        .join('')
...
const renderV3OrderConfirmLetterSvg = (
    ...
    const heroDescLines = wrapText(toText((snapshot as any).hero_desc) || V3_DEFAULT_HERO_DESC, small ? 24 : 38, 2)
