...
export function renderOrderConfirmLetterSvg(snapshot: OrderConfirmLetterSnapshot, small = false) {
    ...
    const texts: string[] = []
    texts.push(`<text x="${width / 2}" y="${y}" text-anchor="middle" font-size="${titleSize}" font-weight="700" fill="#1e2432">${escapeXml(snapshot.title || '订单确认函')}</text>`)
    y += small ? 56 : 100
    ...
    rows.forEach((row, index) => {
        ...
        lines.forEach((line) => {
            texts.push(`<text x="${padding}" y="${y}" font-size="${fontSize}" font-weight="${fontWeight}" fill="#1e2432">${escapeXml(line)}</text>`)
            y += lineHeight
        })
        y += small ? 12 : 20
    })

    texts.push(`<text x="${padding}" y="${y}" font-size="${textSize}" font-weight="600" fill="#1e2432">备注：</text>`)
    y += lineHeight
    const remarkLines = wrapText(snapshot.remark_content || '', small ? 22 : 28, 6)
    remarkLines.forEach((line) => {
        texts.push(`<text x="${padding}" y="${y}" font-size="${textSize}" fill="#5b6475">${escapeXml(line)}</text>`)
        y += lineHeight
    })

    return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="100%" height="100%" fill="#ffffff" rx="24" />${texts.join('')}</svg>`
}

export function buildOrderConfirmLetterDataUrl(snapshot: OrderConfirmLetterSnapshot, small = false) {
