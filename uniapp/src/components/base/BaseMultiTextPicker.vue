<template>
    <view v-if="open" class="base-multi-text-picker" :style="{ zIndex }" @click="handleCancel" @tap="handleCancel" @touchmove.stop.prevent="stopPageTouchMove">
        <BaseOverlayMask :show="open" :z-index="maskZIndex" @close="handleCancel" />
        <view class="base-multi-text-picker__panel" :style="{ zIndex: zIndex + 1 }" @click.stop @tap.stop @touchmove.stop="stopPanelTouchMove">
