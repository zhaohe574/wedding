...
function validateFile(filePath) {
    try {
        ...
        if (uViewComps.length > 0) {
            // 检查是否是需要保留的组件
            const shouldKeep = uViewComps.filter(
                (c) => c.component === 'u-back-top' || c.component === 'u-parse'
            )
            const shouldReplace = uViewComps.filter(
                (c) => c.component !== 'u-back-top' && c.component !== 'u-parse'
            )
