...
function validatePagesJson() {
    ...
    try {
        ...
        if (hasTuniaoEasycom) {
            ...
            const hasBasic =
                content.includes('^tn-(.*)') &&
                content.includes('@tuniao/tnui-vue3-uniapp/components/$1/src/$1.vue')

            if (!hasItemGroup) {
                validationResult.pagesJson.warnings.push(
                    '建议添加 tn-*-item 和 tn-*-group 的 easycom 配置'
                )
            }
            if (!hasBasic) {
        ...
        validationResult.pagesJson.passed =
            hasTuniaoEasycom && !hasUViewConfig && validationResult.pagesJson.errors.length === 0
    } catch (error) {
    ...
    try {
        ...
        if (hasTuniaoStyle) {
            ...
            if (!hasCorrectStylePath) {
                validationResult.appVue.warnings.push(
                    '图鸟 UI 样式路径可能不正确，建议使用: @tuniao/tn-style/dist/uniapp/index.css'
                )
            }
        }
        ...
        if (hasTuniaoIcon) {
            ...
            if (!hasCorrectIconPath) {
                validationResult.appVue.warnings.push(
                    '图鸟 UI 图标路径可能不正确，建议使用: @tuniao/tn-icon/dist/index.css'
                )
            }
        }
        ...
        try {
            ...
            if (config.compilerOptions && config.compilerOptions.types) {
                ...
                if (!types.includes('@tuniao/tnui-vue3-uniapp')) {
                    validationResult.tsconfig.warnings.push(
                        'types 数组中未包含 @tuniao/tnui-vue3-uniapp'
                    )
                }
            }
        } catch (e) {
