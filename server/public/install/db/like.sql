...
('customer_service', 'service_time', '工作日 09:00 - 18:00', 1776200000, 1776200000),
('customer_service', 'tips', '进入微信客服后可继续沟通', 1776200000, 1776200000),
('customer_service', 'wecom_enabled', '0', 1776200000, 1776200000),
('customer_service', 'wecom_corp_id', '', 1776200000, 1776200000),
('customer_service', 'wecom_secret', '', 1776200000, 1776200000),
('customer_service', 'wecom_agent_id', '0', 1776200000, 1776200000),
...
('after_sale', 'auto_callback_plan_days', '7', 1776600000, 1776600000),
...
('staff_settlement_transfer', 'enabled', '0', 1777000000, 1777000000),
('staff_settlement_transfer', 'auto_send', '1', 1777000000, 1777000000),
('staff_settlement_transfer', 'transfer_scene_id', '', 1777000000, 1777000000),
('staff_settlement_transfer', 'transfer_remark', '服务人员结算', 1777000000, 1777000000),
('staff_settlement_transfer', 'user_recv_perception', '服务结算', 1777000000, 1777000000),
('staff_settlement_transfer', 'quota_hint', '单笔转账额度以微信商户平台配置为准，金额达到实名校验阈值时必须配置收款实名。', 1777000000, 1777000000),
('staff_settlement_transfer', 'manual_fallback', '1', 1777000000, 1777000000),
('staff_settlement_transfer', 'amount_name_threshold', '2000', 1777000000, 1777000000),
('staff_settlement_transfer', 'wechatpay_serial', '', 1777000000, 1777000000),
('staff_settlement_transfer', 'wechatpay_public_key', '', 1777000000, 1777000000),
('staff_settlement_transfer', 'transfer_scene_report_infos', '[{"info_type":"岗位","info_content":"服务人员"}]', 1777000000, 1777000000),
('order_confirmation_letter', 'remark_template', '', 1776200000, 1776200000);
COMMIT;
-- ----------------------------
...
('服务人员自动结算', 1, 1, '每分钟生成已完成已付订单的服务人员结算，并处理微信商家转账状态', 'auto_staff_settlement', '', 1, '* * * * *', 1777000000, 1777000000, NULL);
COMMIT;
-- ----------------------------
...
-- ----------------------------
-- Table structure for la_file_cleanup_log
-- ----------------------------
DROP TABLE IF EXISTS `la_file_cleanup_log`;
CREATE TABLE `la_file_cleanup_log` (
  ...
  `items` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '成功明细JSON',
  `failed_detail` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '失败明细JSON',
  `create_time` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `idx_admin_time` (`admin_id`, `create_time`),
  KEY `idx_create_time` (`create_time`)
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '素材清理日志表';
-- ----------------------------
...
(524, 421, 'A', '作品批量审核', '', 0, 'ops.staffWork/batchAudit', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(525, 421, 'A', '证书批量审核', '', 0, 'ops.staffCertificate/batchAudit', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(526, 282, 'A', '批量删除', '', 38, 'ops.staffCertificate/batchDelete', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(527, 282, 'A', '批量审核', '', 48, 'ops.staffCertificate/batchAudit', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(528, 421, 'A', '作品批量删除', '', 0, 'ops.staffWork/batchDelete', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(529, 255, 'A', '批量审核通过', '', 40, 'ops.staffTagReview/batchApprove', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(530, 255, 'A', '批量审核拒绝', '', 50, 'ops.staffTagReview/batchReject', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(531, 421, 'A', '标签批量审核通过', '', 0, 'ops.staffTagReview/batchApprove', '', '', '', '', 0, 0, 0, 1779700000, 1779700000),
(532, 421, 'A', '标签批量审核拒绝', '', 0, 'ops.staffTagReview/batchReject', '', '', '', '', 0, 0, 0, 1779700000, 1779700000)
...
CREATE TABLE `la_user`  (
  ...
  `wechat_risk_rank` tinyint NOT NULL DEFAULT 0 COMMENT '微信检测风险等级：0-4，数字越大风险越高',
  `manual_risk_rank` tinyint NOT NULL DEFAULT -1 COMMENT '后台人工风险等级：-1=跟随微信检测，0-4=人工覆盖',
...
/* 2.0.0 wedding business install baseline: schema */
...
DROP TABLE IF EXISTS `la_service_package_addon`;
...
DROP TABLE IF EXISTS `la_service_city_pool`;
...
DROP TABLE IF EXISTS `la_service_package_region_price`;
...
DROP TABLE IF EXISTS `la_staff_tag_apply`;
...
CREATE TABLE `la_order` (
    ...
    `payment_channel` TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '付款渠道：1=线上支付,2=线下支付',
    `current_confirm_letter_id` INT(11) UNSIGNED NULL DEFAULT NULL COMMENT '当前有效确认函ID',
...
DROP TABLE IF EXISTS `la_order_item_addon`;
...
CREATE TABLE `la_payment` (
    ...
    `transaction_id` VARCHAR(64) NULL DEFAULT NULL COMMENT '第三方交易号',
    `pay_time` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '支付时间',
    `expire_time` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '过期时间',
...
DROP TABLE IF EXISTS `la_order_change_addon`;
...
DROP TABLE IF EXISTS `la_subscribe_message_template`;
CREATE TABLE `la_subscribe_message_template` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
DROP TABLE IF EXISTS `la_subscribe_message_log`;
CREATE TABLE `la_subscribe_message_log` (
...
DROP TABLE IF EXISTS `la_subscribe_message_scene`;
CREATE TABLE `la_subscribe_message_scene` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
CREATE TABLE `la_staff_settlement` (
    ...
    `order_id` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '订单ID',
    `order_item_id` INT UNSIGNED DEFAULT NULL COMMENT '订单项ID',
    `service_date` DATE DEFAULT NULL COMMENT '服务日期',
    ...
    `staff_due_collect_status` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '补收状态：0=无需补收,1=待补收,2=部分补收,3=已补收',
    `staff_due_collect_time` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '最近补收时间',
    `staff_due_collect_admin_id` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '最近补收管理员ID',
    `staff_due_collect_remark` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '最近补收备注',
    ...
    `status` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '状态：0=待结算,1=已结算,2=已取消,3=失败,4=转账处理中/待确认,5=无需打款',
    `settle_time` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '结算时间',
    `settle_way` TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '结算方式：1=余额,2=银行卡,3=微信,4=支付宝,5=无需打款',
...
DROP TABLE IF EXISTS `la_staff_settlement_repay`;
...
DROP TABLE IF EXISTS `la_order_confirm_letter`;
CREATE TABLE `la_order_confirm_letter` (
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `order_id` int(11) UNSIGNED NOT NULL DEFAULT 0 COMMENT '订单ID',
...
DROP TABLE IF EXISTS `la_order_confirm_letter_push_log`;
CREATE TABLE `la_order_confirm_letter_push_log` (
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
DROP TABLE IF EXISTS `la_staff_schedule_confirm_letter_config`;
CREATE TABLE `la_staff_schedule_confirm_letter_config` (
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `staff_id` int(11) UNSIGNED NOT NULL DEFAULT 0 COMMENT '服务人员ID',
  `template_name` varchar(80) NOT NULL DEFAULT '默认海报' COMMENT '模板名称',
  ...
  `show_service_name` tinyint(1) UNSIGNED NOT NULL DEFAULT 1 COMMENT '是否展示服务类型',
    ...
    `design_version` varchar(60) NOT NULL DEFAULT 'staff-schedule-designer-v2' COMMENT '海报设计版本',
...
DROP TABLE IF EXISTS `la_staff_schedule_confirm_letter`;
CREATE TABLE `la_staff_schedule_confirm_letter` (
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `order_id` int(11) UNSIGNED NOT NULL DEFAULT 0 COMMENT '订单ID',
  `staff_id` int(11) UNSIGNED NOT NULL DEFAULT 0 COMMENT '服务人员ID',
  `config_id` int(11) UNSIGNED NOT NULL DEFAULT 0 COMMENT '生成使用的模板配置ID',
  `config_name` varchar(80) NOT NULL DEFAULT '历史配置' COMMENT '生成使用的模板名称',
...
DROP TABLE IF EXISTS `la_monthly_report_material`;
CREATE TABLE `la_monthly_report_material` (
  ...
  `avatar_photo` varchar(500) NOT NULL DEFAULT '' COMMENT '头像素材：执行榜使用',
  `half_body_photo` varchar(500) NOT NULL DEFAULT '' COMMENT '半身素材：单王使用',
...
DROP TABLE IF EXISTS `la_monthly_report_template`;
...
DROP TABLE IF EXISTS `la_monthly_report`;
...
(3, '顾问', 'CRM顾问，仅可处理本人客户和跟进记录', 10, 1776300000, 1776300000, NULL);
INSERT INTO `la_system_menu` (`id`, `pid`, `type`, `name`, `icon`, `sort`, `perms`, `paths`, `component`, `selected`, `params`, `is_cache`, `is_show`, `is_disable`, `create_time`, `update_time`) VALUES
...
(295, 183, 'C', '接单规则', '', 90, 'ops.scheduleRule/lists', 'rule', 'schedule/rule/index', '', '', 0, 1, 0, 1776000100, 1776000100),
(296, 295, 'A', '新增', '', 40, 'ops.scheduleRule/add', '', '', '', '', 0, 0, 0, 1776000100, 1776000100),
(297, 295, 'A', '编辑', '', 30, 'ops.scheduleRule/edit', '', '', '', '', 0, 0, 0, 1776000100, 1776000100),
(298, 295, 'A', '删除', '', 20, 'ops.scheduleRule/delete', '', '', '', '', 0, 0, 0, 1776000100, 1776000100),
(299, 295, 'A', '状态切换', '', 10, 'ops.scheduleRule/changeStatus', '', '', '', '', 0, 0, 0, 1776000100, 1776000100),
(354, 183, 'C', '吉日设置', '', 85, 'ops.calendarEvent/lists', 'lucky-day', 'schedule/lucky_day/index', '', '', 0, 1, 0, 1776700000, 1776700000),
(355, 354, 'A', '吉日列表', '', 0, 'ops.calendarEvent/lists', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(356, 354, 'A', '吉日详情', '', 0, 'ops.calendarEvent/detail', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(357, 354, 'A', '保存吉日', '', 0, 'ops.calendarEvent/save', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(358, 354, 'A', '删除吉日', '', 0, 'ops.calendarEvent/delete', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(359, 354, 'A', '批量设置吉日', '', 0, 'ops.calendarEvent/batchSave', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(360, 354, 'A', '拥堵等级选项', '', 0, 'ops.calendarEvent/congestionLevelOptions', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
...
(497, 0, 'M', '快捷工具', 'el-icon-Tools', 780, '', 'quick-tools', '', '', '', 0, 1, 0, 1778000100, 1778000100),
...
(391, 199, 'A', '重试转账', '', 0, 'finance.settlement/retryTransfer', '', '', '', '', 0, 0, 0, 1777000000, 1777000000),
(392, 199, 'A', '同步转账状态', '', 0, 'finance.settlement/syncTransfer', '', '', '', '', 0, 0, 0, 1777000000, 1777000000),
...
(334, 207, 'A', '售后统计', '', 0, 'ops.aftersaleTicket/statistics', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(335, 207, 'A', '趋势数据', '', 0, 'ops.aftersaleTicket/trend', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(336, 207, 'A', '工单详情', '', 0, 'ops.aftersaleTicket/ticketDetail', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(337, 207, 'A', '创建工单', '', 0, 'ops.aftersaleTicket/createTicket', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(338, 207, 'A', '分配工单', '', 0, 'ops.aftersaleTicket/assignTicket', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(339, 207, 'A', '处理工单', '', 0, 'ops.aftersaleTicket/handleTicket', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(340, 207, 'A', '关闭工单', '', 0, 'ops.aftersaleTicket/closeTicket', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(341, 207, 'A', '升级工单', '', 0, 'ops.aftersaleTicket/escalateTicket', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(342, 207, 'A', '工单日志', '', 0, 'ops.aftersaleTicket/ticketLogs', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(343, 207, 'A', '投诉列表', '', 0, 'ops.aftersaleTicket/complaintLists', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(344, 207, 'A', '投诉详情', '', 0, 'ops.aftersaleTicket/complaintDetail', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(345, 207, 'A', '处理投诉', '', 0, 'ops.aftersaleTicket/handleComplaint', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(346, 207, 'A', '回访列表', '', 0, 'ops.aftersaleTicket/callbackLists', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(347, 207, 'A', '回访详情', '', 0, 'ops.aftersaleTicket/callbackDetail', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(348, 207, 'A', '创建回访', '', 0, 'ops.aftersaleTicket/createCallback', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(349, 207, 'A', '完成回访', '', 0, 'ops.aftersaleTicket/completeCallback', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(350, 207, 'A', '标记无法联系', '', 0, 'ops.aftersaleTicket/markUnreachable', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(351, 207, 'A', '问题升级', '', 0, 'ops.aftersaleTicket/escalateProblem', '', '', '', '', 0, 0, 0, 1776500000, 1776500000),
(352, 207, 'A', '获取售后设置', '', 0, 'ops.aftersaleTicket/getConfig', '', '', '', '', 0, 0, 0, 1776600000, 1776600000),
(353, 207, 'A', '保存售后设置', '', 0, 'ops.aftersaleTicket/setConfig', '', '', '', '', 0, 0, 0, 1776600000, 1776600000),
...
(447, 0, 'M', '新人问卷', 'el-icon-Document', 560, '', 'couple-questionnaire', '', '', '', 0, 1, 0, 1777600000, 1777600000),
(448, 447, 'C', '问卷列表', '', 100, 'growth.coupleQuestionnaire/lists', 'lists', 'questionnaire/couple_questionnaire/index', '', '', 0, 1, 0, 1777600000, 1777600000),
(449, 447, 'C', '配置问卷', '', 95, 'growth.coupleQuestionnaire/detail', 'config', 'questionnaire/couple_questionnaire/config', '/couple-questionnaire/lists', '', 0, 0, 0, 1777600000, 1777600000),
(450, 448, 'A', '保存问卷草稿', '', 0, 'growth.coupleQuestionnaire/save', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(451, 448, 'A', '发布问卷新版', '', 0, 'growth.coupleQuestionnaire/publish', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(452, 447, 'C', '基础题库', '', 90, 'growth.coupleQuestionBank/lists', 'bank', 'questionnaire/couple_question_bank/index', '', '', 0, 1, 0, 1777600000, 1777600000),
(453, 452, 'A', '题库详情', '', 0, 'growth.coupleQuestionBank/detail', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(454, 452, 'A', '保存题库', '', 0, 'growth.coupleQuestionBank/save', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(455, 452, 'A', '删除题库', '', 0, 'growth.coupleQuestionBank/delete', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(456, 452, 'A', '题库状态', '', 0, 'growth.coupleQuestionBank/changeStatus', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(457, 452, 'A', '题库排序', '', 0, 'growth.coupleQuestionBank/sort', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(458, 447, 'C', '问卷任务', '', 80, 'growth.coupleQuestionnaire/tasks', 'tasks', 'questionnaire/couple_questionnaire_task/index', '', '', 0, 1, 0, 1777600000, 1777600000),
(459, 458, 'A', '任务详情', '', 0, 'growth.coupleQuestionnaire/taskDetail', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(460, 458, 'A', '发送问卷', '', 0, 'growth.coupleQuestionnaire/send', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
...
(477, 223, 'C', '我的作品', '', 94, 'ops.staffWork/lists', 'work', 'staff_center/work/index', '', '', 0, 1, 0, 1777900000, 1777900000),
(478, 223, 'C', '我的证书', '', 93, 'ops.staffCertificate/lists', 'certificate', 'staff_center/certificate/index', '', '', 0, 1, 0, 1777900000, 1777900000),
...
(441, 223, 'C', '新人问卷', '', 37, 'ops.staff/myCoupleQuestionnaireConfig', 'couple-questionnaire', 'staff_center/couple_questionnaire/index', '', '', 0, 1, 0, 1777600000, 1777600000),
(442, 441, 'A', '保存问卷草稿', '', 0, 'ops.staff/myCoupleQuestionnaireSave', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(443, 441, 'A', '发布问卷新版', '', 0, 'ops.staff/myCoupleQuestionnairePublish', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(444, 441, 'A', '问卷任务列表', '', 0, 'ops.staff/myCoupleQuestionnaireTasks', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(445, 441, 'A', '问卷任务详情', '', 0, 'ops.staff/myCoupleQuestionnaireTaskDetail', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
(446, 441, 'A', '发送问卷', '', 0, 'ops.staff/myCoupleQuestionnaireSend', '', '', '', '', 0, 0, 0, 1777600000, 1777600000),
...
(361, 223, 'C', '我的售后', '', 35, 'ops.aftersaleTicket/myTicketLists', 'aftersale', 'staff_center/aftersale/index', '', '', 0, 1, 0, 1776700000, 1776700000),
(362, 361, 'A', '我的工单列表', '', 0, 'ops.aftersaleTicket/myTicketLists', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(363, 361, 'A', '我的工单详情', '', 0, 'ops.aftersaleTicket/myTicketDetail', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(364, 361, 'A', '处理我的工单', '', 0, 'ops.aftersaleTicket/myHandleTicket', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(365, 361, 'A', '我的投诉列表', '', 0, 'ops.aftersaleTicket/myComplaintLists', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(366, 361, 'A', '我的投诉详情', '', 0, 'ops.aftersaleTicket/myComplaintDetail', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
(367, 361, 'A', '我的回访列表', '', 0, 'ops.aftersaleTicket/myCallbackLists', '', '', '', '', 0, 0, 0, 1776700000, 1776700000),
...
(263, 28, 'C', '订阅消息', 'el-icon-Bell', 53, 'growth.subscribe/templateList', 'subscribe-message', 'subscribe/template/index', '', '', 0, 0, 1, 1775001800, 1775001800),
...
(301, 0, 'M', 'CRM管理', 'el-icon-User', 620, '', 'crm', '', '', '', 0, 1, 0, 1776300000, 1776300000),
(302, 301, 'C', '顾问管理', '', 90, 'crm.salesAdvisor/lists', 'advisor', 'crm/advisor/index', '', '', 0, 1, 0, 1776300000, 1776300000),
(303, 302, 'A', '新增', '', 40, 'crm.salesAdvisor/add', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(304, 302, 'A', '编辑', '', 30, 'crm.salesAdvisor/edit', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(305, 302, 'A', '删除', '', 20, 'crm.salesAdvisor/delete', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(306, 302, 'A', '状态切换', '', 10, 'crm.salesAdvisor/changeStatus', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(307, 302, 'A', '客户数校准', '', 9, 'crm.salesAdvisor/syncCustomerCount', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(308, 301, 'C', '客户管理', '', 100, 'crm.customer/lists', 'customer', 'crm/customer/index', '', '', 0, 1, 0, 1776300000, 1776300000),
(309, 308, 'A', '详情', '', 40, 'crm.customer/detail', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(310, 308, 'A', '编辑', '', 30, 'crm.customer/edit', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(311, 308, 'A', '转移顾问', '', 20, 'crm.customer/transferAdvisor', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(312, 308, 'A', '顾问选项', '', 10, 'crm.customer/advisorOptions', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(313, 308, 'A', '客户选项', '', 9, 'crm.customer/options', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(314, 301, 'C', '跟进记录', '', 80, 'crm.followRecord/lists', 'follow-record', 'crm/follow-record/index', '', '', 0, 1, 0, 1776300000, 1776300000),
(315, 314, 'A', '详情', '', 30, 'crm.followRecord/detail', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(316, 314, 'A', '新增', '', 20, 'crm.followRecord/add', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(317, 314, 'A', '选项', '', 10, 'crm.followRecord/options', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(318, 314, 'A', '客户选项', '', 9, 'crm.followRecord/customerOptions', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(319, 301, 'C', '流失预警', '', 70, 'crm.lossWarning/lists', 'loss-warning', 'crm/loss-warning/index', '', '', 0, 1, 0, 1776300000, 1776300000),
(320, 319, 'A', '详情', '', 60, 'crm.lossWarning/detail', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(321, 319, 'A', '处理', '', 50, 'crm.lossWarning/handle', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(322, 319, 'A', '忽略', '', 40, 'crm.lossWarning/ignore', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(323, 319, 'A', '生成预警', '', 30, 'crm.lossWarning/generate', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(324, 319, 'A', '推送预警', '', 20, 'crm.lossWarning/push', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(325, 319, 'A', '统计', '', 10, 'crm.lossWarning/stats', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(326, 319, 'A', '选项', '', 9, 'crm.lossWarning/options', '', '', '', '', 0, 0, 0, 1776300000, 1776300000),
(327, 28, 'C', '确认函字体资源', 'el-icon-DocumentChecked', 52, 'setting.order_confirm_letter/fontLists', 'order_confirm_letter', 'setting/order_confirm_letter/index', '', '', 0, 1, 0, 1776400000, 1776400000),
(328, 327, 'A', '保存设置', '', 0, 'setting.order_confirm_letter/setConfig', '', '', '', '', 0, 0, 0, 1776400000, 1776400000),
(329, 327, 'A', '字体列表', '', 0, 'setting.order_confirm_letter/fontLists', '', '', '', '', 0, 0, 0, 1776400000, 1776400000),
(330, 327, 'A', '上传字体', '', 0, 'setting.order_confirm_letter/uploadFont', '', '', '', '', 0, 0, 0, 1776400000, 1776400000),
(331, 327, 'A', '设置字体', '', 0, 'setting.order_confirm_letter/setFont', '', '', '', '', 0, 0, 0, 1776400000, 1776400000),
(332, 327, 'A', '删除字体', '', 0, 'setting.order_confirm_letter/deleteFont', '', '', '', '', 0, 0, 0, 1776400000, 1776400000),
(333, 327, 'A', '检测字体', '', 0, 'setting.order_confirm_letter/checkFont', '', '', '', '', 0, 0, 0, 1776400000, 1776400000);
